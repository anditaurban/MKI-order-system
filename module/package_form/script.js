...
async function loadDetailSales(Id) {
  ...
  fetch(`${baseUrl}/detail/sales_package/${Id}`, {
    ...
    .then((detail) => {
      ...
      document.getElementById('totalBerat').innerText = `${detail.total_weight?.toLocaleString('id-ID') || 0} gram`;
      document.getElementById('totalItem').innerText = `${detail.total_qty?.toLocaleString('id-ID') || 0} item`;
      ...
      detail.items.forEach(item => {
        ...
        // Pilih option berdasarkan product_id jika ada
        const select = row.querySelector(".itemNama");
        const match = Array.from(select.options).find(o => o.value == item.product_id);
        if (match) select.value = match.value;

        // Tambah sub-item jika ada
        if (Array.isArray(item.item_detail)) {
        
          
          item.item_detail.forEach(sub => {
            tambahSubItem(row.querySelector('button')); // Gunakan tombol dari baris ini sebagai referensi
            const subRow = row.nextElementSibling; // karena langsung ditambahkan setelah row utama

            subRow.querySelector(".searchProduk").value = sub.item_name;
            subRow.querySelector(".itemNama").value = sub.item_id;
            // subRow.querySelector(".itemCat").innerText = sub.category;
            subRow.querySelector(".itemCat").innerText = 'Item Kemitraan';
            subRow.querySelector(".itemQty").value = sub.qty;
            subRow.querySelector(".itemBerat").innerText = sub.weight?.toLocaleString('id-ID') || 0;

            const subSelect = subRow.querySelector(".itemNama");
            const subMatch = Array.from(subSelect.options).find(o => o.value == sub.product_id);
            if (subMatch) subSelect.value = subMatch.value;
            document.getElementById("btnTambahSubItem").classList.remove("hidden");
          });
        }
      });
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const mainItem = {
        ...
        qty: (cat === 'Kemitraan') ? 1 : parseInt(row.querySelector('.itemQty')?.value || 0),
      };

      const item_detail = [];

      while (i + 1 < rows.length && rows[i + 1].classList.contains('sub-item-row')) {
    ...
    // const body = { sales_package_detail };
    console.log('PAYLOAD:', JSON.stringify(body, null, 2));
    

    const res = await fetch(`${baseUrl}/update/sales_package/${window.detail_id}`, {
      method: 'PUT',
      headers: {
    ...
    if (res.ok) {
      Swal.fire('Sukses', '✅ Data packing list berhasil diperbarui.', 'success');
      loadModuleContent('package');
    } else {
      Swal.fire('Gagal', json.message || '❌ Gagal update packing list.', 'error');
    }
  } catch (error) {
    console.error(error);
...
function recalculateTotal() {
  ...
  document.getElementById('totalItem').innerText = `${itemqty.toLocaleString('id-ID')} item`;
}
